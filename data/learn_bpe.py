...
def replace_pair(pair, vocab, indices):
    ...
    pair_str = ''.join(pair)  # AB
    pair_str = pair_str.replace('\\','\\\\')  # AB
    changes = []
    ...
    for j, freq in iterator:
        if freq < 1:
            continue
        word, freq = vocab[j]  # ("C", 'A', 'B', 'D', 'A', 'B</w>'), 1
        new_word = ' '.join(word)  # C A B D A B</w>
        new_word = pattern.sub(pair_str, new_word)  # 在new_word中匹配"A B", 如果匹配到了，则将匹配项替换为AB
        new_word = tuple(new_word.split(' '))  # ("C", "AB", "D", "A", "B</w>")
...
def get_vocabulary(spacy_model_name, text_filepath):
    """
    build vocabulary from text files with spacy
    """
...
def learn_bpe(infile_names, outfile_name, num_symbols, min_frequency=2, verbose=False, is_dict=False, total_symbols=False):
    ...
    # vocab = get_vocabulary(infile_names)
    vocab = Counter()
